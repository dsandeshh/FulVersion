...
function countDownTimer(){
    ...
    timer.innerHTML=`<div>${playerjs}:${secs}</div>`;
    timer.style.color='green'
    //10-sec condition
	if(remainingTime>10000){
		semicircle[1].style.backgroundColor='green';
		semicircle[0].style.backgroundColor='green';
	}
    if(remainingTime<=10000){
...
function secondchangeNext(){
	let changeTime=Date.now()+1;
	futureTime=changeTime+setTime;
...
// Starts about confetti aboout confetti
var confetti = {
...
//set confetti location from these two functions
// start();
// stop();
